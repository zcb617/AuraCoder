...
impl Engine for CodexEngine {
    ...
    async fn start_thread(
        ...
        if let Some(service) = self.auracoder_thread_mcp_service() {
            let auracoder_tools = service.dynamic_tools_spec();
            if let (serde_json::Value::Array(existing), serde_json::Value::Array(auracoder)) =
                (&mut dynamic_tools, auracoder_tools)
            {
                existing.extend(auracoder);
            }
        }
              ...
              incoming = incoming_rx.recv() => {
                match incoming {
                  ...
                  IncomingMessage::Request { id, raw_id, method, params } => {
                    ...
                    // 子代理授权请求不在 AuraCoder 中独立管理，只处理主线程请求。
                    // 旧逻辑保留，不执行，已由子代理来源分支替代：
                    // if !belongs_to_thread(&params, &thread_id) {
                    ...
                    if method_signature(&method) == "accountchatgptauthtokensrefresh" {
                        ...
                        log::warn!(
                            "codex requested external ChatGPT token refresh, but AuraCoder does not manage chatgptAuthTokens mode"
                        );
                        self
                        ...
                        .respond_error(
                          &raw_id,
                          -32601,
                          "`account/chatgptAuthTokens/refresh` is not supported by AuraCoder",
                    ...
                    if method_signature(&method) == "itemtoolcall" {
                        ...
                        if let Some(namespace) = namespace.as_deref() {
                            if namespace != "auracoder_computer_control"
                                && namespace != crate::auracoder_thread_mcp_service::PANES_THREAD_NAMESPACE
                            {
                                transport
                                    .respond_success(
                                        &raw_id,
                        ...
                        if namespace.as_deref()
                            == Some(crate::auracoder_thread_mcp_service::PANES_THREAD_NAMESPACE)
                        {
                            let tool = raw_tool
                                .strip_prefix("auracoder_thread.")
                                .unwrap_or(&raw_tool)
                                .to_string();
                            ...
                            let response = match self.auracoder_thread_mcp_service() {
                        ...
                        let tool = raw_tool
                            .strip_prefix("auracoder_computer_control.")
                            .unwrap_or(&raw_tool)
                            .to_string();
                        ...
                        let response = match self.computer_control_service() {
                            ...
                            None => crate::computer_control_service::dynamic_tool_failure(
                                "authorization_required: AuraCoder 电脑操作服务尚未就绪",
                            ),
                        };
                        transport.respond_success(&raw_id, response).await.ok();
                        continue;
...
impl CodexEngine {
    ...
    pub fn set_auracoder_thread_mcp_service(
        &self,
        service: Arc<crate::auracoder_thread_mcp_service::AuraCoderThreadMcpService>,
    ) {
    ...
    /// 获取当前配置的 AuraCoder 本地会话读取工具服务。
    fn auracoder_thread_mcp_service(
        &self,
    ) -> Option<Arc<crate::auracoder_thread_mcp_service::AuraCoderThreadMcpService>> {
        self.auracoder_thread_mcp_service
            .lock()
            .ok()
            .and_then(|service| service.clone())
    }
                ...
                match incoming {
                  ...
                  IncomingMessage::Request { id, raw_id, method, params } => {
                    ...
                    if method_signature(&method) == "accountchatgptauthtokensrefresh" {
                        ...
                        log::warn!(
                            "codex requested external ChatGPT token refresh during review, but AuraCoder does not manage chatgptAuthTokens mode"
                        );
                        self
                            .publish_external_auth_tokens_warning(
                                previous_account_id.clone(),
    ...
    pub async fn sandbox_preflight_warning(&self) -> Option<String> {
        ...
        if prefer_external_sandbox_by_default() {
            Some(
                "AuraCoder is forcing Codex external sandbox mode on macOS to avoid opaque tool-call failures in local workspace-write mode. Set `PANES_CODEX_PREFER_WORKSPACE_WRITE=1` only for diagnostics."
                    .to_string(),
            )
        } else {
            Some(
                "macOS denied Codex local sandbox (`sandbox-exec`). Commands may fail unless AuraCoder uses external sandbox mode. This is an OS/policy restriction, not a promptable permission.".to_string(),
            )
        }
    }
...
fn codex_unavailable_details_for_platform(
    ...
    match (platform, resolution.login_shell_executable.as_ref()) {
        ("macos", Some(shell_path)) => Some(format!(
            "Codex was found in your login shell at `{}`, but AuraCoder does not see this in its app PATH. This is common when launching from Finder on macOS. App PATH: `{}`",
            shell_path.display(),
            path_preview
        )),
        ...
        (_, Some(shell_path)) => Some(format!(
            "Codex was found in your login shell at `{}`, but AuraCoder does not see this in its app PATH. App PATH: `{}`",
            shell_path.display(),
            path_preview
        )),
        (_, None) => Some(format!(
...
fn codex_execution_failure_details_for_platform(
        ...
        .contains("env: node: no such file or directory")
    {
        if platform == "windows" {
            return format!(
                "Codex executable was found at `{executable}`, but AuraCoder could not find `node` when launching it. This usually means Node.js is not installed or its install directory is missing from PATH on Windows. App PATH: `{path_preview}`. Error: {error}"
            );
        }

        if platform != "macos" {
            return format!(
                "Codex executable was found at `{executable}`, but AuraCoder could not find `node` when launching it. App PATH: `{path_preview}`. Error: {error}"
            );
        }

        return format!(
            "Codex executable was found at `{executable}`, but AuraCoder could not find `node` when launching it (Finder-launched apps often have a limited PATH). App PATH: `{path_preview}`. Error: {error}"
        );
    }

    format!(
        "Codex executable was found at `{executable}`, but AuraCoder could not run it. App PATH: `{path_preview}`. Error: {error}"
    )
}
...
fn codex_fix_commands_for_platform(
        ...
        if resolution.executable.is_none() {
            if let Some(shell_path) = &resolution.login_shell_executable {
                if let Some(bin_dir) = shell_path.parent() {
                    ...
                    fixes.push("open -a AuraCoder".to_string());
                }
            } else {
    ...
    if platform == "windows" {
        ...
        if resolution.executable.is_none() {
            ...
            fixes.push(
                "Ensure `%APPDATA%\\npm` is present in PATH, then restart AuraCoder.".to_string(),
            );
            return fixes;
        }
        ...
        if execution_error.is_some() {
            ...
            fixes.push(
                "Ensure Node.js 20+ is installed and visible to AuraCoder, then restart the app."
                    .to_string(),
            );
        }
        return fixes;
...
fn build_reconciled_turn_completion_events(
        ...
        && reconciled.status == TurnCompletionStatus::Completed
    {
        events.push(EngineEvent::Error {
            message:
                "Codex finished after AuraCoder lost the live event stream, so the transcript may be incomplete."
                    .to_string(),
            recoverable: true,
        });
...
fn thread_runtime_from_start_response(
    ...
    let mut runtime = ThreadRuntime {
        ...
        // The requested AuraCoder effort is authoritative for the subsequent turn/start.
    ...
    // 旧逻辑保留在注释中：线程创建响应返回值不能覆盖 AuraCoder 本轮请求值。
...
fn unsupported_external_auth_tokens_message(
    ...
    match (previous_account_id, reason) {
        (Some(previous_account_id), Some(reason)) => format!(
            "Codex is using external ChatGPT auth tokens for account `{previous_account_id}` after `{reason}`, but AuraCoder cannot refresh those tokens. Re-authenticate outside AuraCoder or switch Codex to a managed auth mode."
        ),
        (Some(previous_account_id), None) => format!(
            "Codex is using external ChatGPT auth tokens for account `{previous_account_id}`, but AuraCoder cannot refresh those tokens. Re-authenticate outside AuraCoder or switch Codex to a managed auth mode."
        ),
        (None, Some(reason)) => format!(
            "Codex is using external ChatGPT auth tokens after `{reason}`, but AuraCoder cannot refresh those tokens. Re-authenticate outside AuraCoder or switch Codex to a managed auth mode."
        ),
        (None, None) => "Codex is using external ChatGPT auth tokens, but AuraCoder cannot refresh those tokens. Re-authenticate outside AuraCoder or switch Codex to a managed auth mode.".to_string(),
    }
}
...
async fn refresh_protocol_diagnostics_via_transport(
    ...
    let (
        ...
        // Apps/连接器不属于 AuraCoder 管理的 Skill、Plugin、MCP 能力范围，不再请求 app/list。
...
mod tests {
    ...
    async fn remote_text_attachment_uses_remote_path_without_local_file_read() {
        let attachment = TurnAttachment {
            ...
            is_remote: true,
            remote_text_content: Some("AuraCoder SSH 远端附件验证码：ATTACHMENT-PLAN4".to_string()),
        };
        ...
        assert!(items.iter().any(|item| {
            item.get("type").and_then(Value::as_str) == Some("text")
                && item
                    ...
                    .is_some_and(|text| {
                        text.contains(
                            "/home/tester/.cache/auracoder/attachments/workspace/thread/file.md",
    ...
    #[test]
    fn thread_start_params_register_auracoder_computer_control_namespace() {
        let dynamic_tools = json!([
            {
                "type": "namespace",
                "name": "auracoder_computer_control",
                "tools": []
            }
        ]);
        ...
        let details = codex_execution_failure_details_for_platform(
            ...
            &CodexExecutableResolution {
                executable: Some(std::path::PathBuf::from(
                    r"C:\Users\auracoder\AppData\Roaming\npm\codex.cmd",
                )),
                source: "app-path",
                app_path: Some(r"C:\Windows\System32".to_string()),
                login_shell_executable: None,
        ...
        let mapped = map_skill_entries(&[
            json!({
                ...
                "skills": [
                    ...
                    {
                        "name": "user-skill",
                        "path": "/Users/auracoder/.codex/user-skill",
    ...
    fn map_config_state_uses_layers_and_structured_values() {
            ...
            "layers": [
                {
                    "name": {
                        "type": "user",
                        "file": "/Users/auracoder/.codex/config.toml"
                    },
                    "version": "v2",
                    "config": {}
                }
        ...
        assert_eq!(
            ...
            "user:/Users/auracoder/.codex/config.toml"
        );
