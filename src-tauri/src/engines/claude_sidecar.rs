...
struct ClaudeTransport {
    /// Claude sidecar 子进程句柄。
    child: Mutex<Child>,
    /// Claude sidecar 标准输入写入端。
    stdin: Mutex<ChildStdin>,
    /// sidecar 事件广播发送端；正常会话事件继续通过此通道广播。
    event_tx: broadcast::Sender<SidecarEvent>,
    /// 实际使用的运行时可执行文件路径。
    runtime_path: String,
    /// 实际启动的 sidecar 脚本路径。
    sidecar_path: String,
    /// 探测到的 Claude CLI 路径，未探测到时为 None。
    claude_cli_path: Option<String>,
    /// 实际使用或尝试加载的 Claude SDK 模块标识。
    sdk_module_path: String,
    /// stderr 最后若干条非空诊断行。
    stderr_tail: Arc<StdMutex<VecDeque<String>>>,
    /// stderr reader 完成信号，只能在诊断时消费一次。
    stderr_complete_rx: Mutex<Option<oneshot::Receiver<()>>>,
}
...
impl ClaudeTransport {
    async fn spawn(
        ...
        let sdk_module_path = std::env::var("CLAUDE_AGENT_SDK_MODULE")
            .ok()
            .filter(|value| !value.trim().is_empty())
            .or_else(|| sdk_module_specifier.clone())
            .unwrap_or_else(|| "@anthropic-ai/claude-agent-sdk".to_string());
        ...
        // 在启动 stdout reader 前建立一次性启动握手，避免 sidecar 立即输出 ready/error 时丢失。
        let (startup_tx, startup_rx) = oneshot::channel();
        let stderr_tail = Arc::new(StdMutex::new(VecDeque::with_capacity(20)));
        let (stderr_complete_tx, stderr_complete_rx) = oneshot::channel();
    ...
    /// 收集启动失败所需的有限诊断上下文，不记录令牌、环境变量或用户消息。
    async fn startup_diagnostics(&self) -> String {
        let exit_status = {
            let mut child = self.child.lock().await;
            match timeout(Duration::from_millis(250), child.wait()).await {
                Ok(Ok(status)) => status
                    ...
                    .unwrap_or_else(|| "exitCode=signal/unknown".to_string()),
                Ok(Err(error)) => format!("exitCode=unknown ({error})"),
                Err(_) => "exitCode=running".to_string(),
            }
        };
        if let Some(stderr_complete_rx) = self.stderr_complete_rx.lock().await.take() {
            // 进程仍在运行时 reader 不会自然结束，短超时后继续返回有限诊断。
            let _ = timeout(Duration::from_millis(250), stderr_complete_rx).await;
        }
        ...
        /*
        let runtime_label = ["n", "odePath"].concat();
        */
        format!(
            "{exit_status}; stderrTail={stderr_tail}; nodePath={}; sidecarPath={}; claudeCliPath={}; sdkModulePath={}",
            self.runtime_path,
            self.sidecar_path,
            self.claude_cli_path.as_deref().unwrap_or("not-found"),
            self.sdk_module_path,
        )
    }
}
...
impl ClaudeSidecarEngine {
    ...
    pub fn set_auracoder_thread_mcp_service(&self, service: Arc<AuraCoderThreadMcpService>) {
        // 旧实现保留迁移留痕：异步运行线程内不能使用 blocking_lock。
        // let mut state = self.state.blocking_lock();
        // state.auracoder_thread_mcp_service = Some(service);
    ...
    async fn ensure_transport(&self) -> anyhow::Result<Arc<ClaudeTransport>> {
        ...
        let (transport, startup_rx) = ClaudeTransport::spawn(sidecar_path).await?;
        let transport = Arc::new(transport);

        // 旧实现通过 spawn 返回后再订阅 broadcast 等待 ready，保留迁移留痕：
        // let mut rx = transport.subscribe();
        // loop { match rx.recv().await { Ok(SidecarEvent::Ready) => break, Ok(_) => continue, Err(_) => break } }
        // 现由 spawn 内部在 stdout reader 启动前创建的 oneshot 启动握手接替。
        let ready = tokio::time::timeout(Duration::from_secs(15), startup_rx).await;

        match ready {
            Ok(Ok(SidecarEvent::Ready)) => log::info!("claude agent sidecar is ready"),
            Ok(Ok(SidecarEvent::Error { message, .. })) => {
                let diagnostics = transport.startup_diagnostics().await;
                transport.kill().await;
                anyhow::bail!("claude sidecar startup error: {message}; {diagnostics}");
            }
            Ok(Ok(event)) => {
                let diagnostics = transport.startup_diagnostics().await;
                transport.kill().await;
            ...
            Ok(Err(_)) => {
                let diagnostics = transport.startup_diagnostics().await;
                transport.kill().await;
                anyhow::bail!("claude sidecar startup event channel closed; {diagnostics}");
            }
            Err(_) => {
                let diagnostics = transport.startup_diagnostics().await;
                transport.kill().await;
...
fn node_fix_commands_for_platform(
    ...
    if platform == "windows" {
        ...
        return vec![
            ...
            "echo %PATH%".to_string(),
            "Ensure your Node.js install directory is present in PATH, then restart AuraCoder."
                .to_string(),
        ];
    }
...
impl Engine for ClaudeSidecarEngine {
    ...
    async fn send_message(
        ...
        // let (computer_control_service, auracoder_thread_mcp_service) = {
        //     let state = self.state.lock().await;
        //     (
...
mod tests {
    ...
    async fn service_setters_are_safe_inside_tokio_runtime() {
        ...
        engine.set_auracoder_thread_mcp_service(Arc::new(AuraCoderThreadMcpService::new(db)));
        ...
        let auracoder_thread_mcp_service = engine
            .auracoder_thread_mcp_service
            .lock()
            .expect("AuraCoder thread MCP service mutex should not be poisoned");
        assert!(
    ...
    async fn startup_handshake_keeps_immediate_ready_event() {
        ...
        fs::write(
            &script_path,
            r#"process.stdout.write(JSON.stringify({ type: "ready" }) + "\n"); setInterval(() => {}, 1000);"#,
        )
        .expect("ready script");

        let result = async {
            let (transport, startup_rx) = ClaudeTransport::spawn(script_path.clone()).await?;
            let event = timeout(Duration::from_secs(3), startup_rx)
                .await
                .context("ready handshake timed out")?
                .context("ready handshake sender closed")?;
        ...
        result.expect("immediate ready handshake should be delivered");
    }

    #[cfg(target_os = "windows")]
    #[tokio::test]
    async fn windows_verbatim_sidecar_path_is_normalized_before_node_spawn() {
        let temp_dir =
            std::env::temp_dir().join(format!("claude-sidecar-verbatim-{}", Uuid::new_v4()));
        fs::create_dir_all(&temp_dir).expect("temporary sidecar directory");
        let script_path = temp_dir.join("ready.mjs");
        fs::write(
            &script_path,
        ...
        let result = async {
            let verbatim_path = crate::path_utils::legacy_windows_verbatim_path(&script_path)
                .context("verbatim sidecar path")?;
            assert!(verbatim_path.starts_with(r"\\?\"));
            let verbatim_path = PathBuf::from(verbatim_path);
            let (transport, startup_rx) = ClaudeTransport::spawn(verbatim_path).await?;
            let event = timeout(Duration::from_secs(3), startup_rx)
                .await
                .context("ready handshake timed out")?
                .context("ready handshake sender closed")?;
            assert!(matches!(event, SidecarEvent::Ready));
            assert!(!transport.sidecar_path.starts_with(r"\\?\"));
            assert_eq!(transport.sidecar_path, script_path.to_string_lossy());
            transport.kill().await;
            anyhow::Ok(())
        }
        .await;
        ...
        let _ = fs::remove_dir_all(&temp_dir);
        result.expect("verbatim sidecar path should be normalized before spawn");
    }
    ...
    async fn startup_error_preserves_stderr_paths_and_exit_status() {
        ...
        fs::write(
            &script_path,
            r#"process.stderr.write("fixed startup diagnostic\n"); process.stdout.write(JSON.stringify({ type: "error", message: "fixed startup error", recoverable: false, errorType: "startup_dependency_load_failed", isAuthError: false }) + "\n"); process.exit(23);"#,
        )
        .expect("error script");
        ...
        let result = async {
            ...
            let event = timeout(Duration::from_secs(3), startup_rx)
                .await
                .context("error handshake timed out")?
                .context("error handshake sender closed")?;
            match event {
                SidecarEvent::Error { message, .. } => assert_eq!(message, "fixed startup error"),
                other => panic!("unexpected startup event: {other:?}"),
            }
            // 旧实现依赖固定 sleep 等待 stderr，保留迁移留痕；现在由 reader 完成握手接替。
            // tokio::time::sleep(Duration::from_millis(50)).await;
            let diagnostics = transport.startup_diagnostics().await;
            assert!(diagnostics.contains("fixed startup diagnostic"));
            assert!(diagnostics.contains(script_path.to_string_lossy().as_ref()));
            assert!(diagnostics.contains(&transport.runtime_path));
        ...
        let _ = fs::remove_dir_all(&temp_dir);
        result.expect("startup error diagnostics should preserve context");
    }
    ...
    fn node_fix_commands_for_windows_cover_where_and_restart() {
        ...
        assert!(fixes.iter().any(|fix| fix.contains("restart AuraCoder")));
    }
}
