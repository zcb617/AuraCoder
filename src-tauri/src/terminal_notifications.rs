...
const CODEX_NOTIFY_CONFIG_OVERRIDE: &str = r#"notify=["auracoder","codex-notify"]"#;
...
const CLAUDE_HOOK_COMMAND: &str = "auracoder claude-hook";
const NOTIFICATION_DEFAULT_TITLE: &str = "AuraCoder";
...
pub struct AgentNotificationSettingsStatusDto {
    ...
    pub notification_sound_options: Vec<String>,
    pub sound_preview_supported: bool,
...
pub fn agent_notification_settings_status() -> anyhow::Result<AgentNotificationSettingsStatusDto> {
    let config = crate::config::app_config::AppConfig::load_or_create()
        .context("failed to load AuraCoder config")?;
    let sound_capability = crate::config::app_config::notification_sound_capability();
    ...
    Ok(AgentNotificationSettingsStatusDto {
        ...
        notification_sound_options: sound_capability.options,
        sound_preview_supported: sound_capability.preview_supported,
        claude,
        codex,
    })
}
...
pub fn maybe_handle_cli_subcommand() -> anyhow::Result<bool> {
    ...
    match subcommand.as_str() {
        ...
        CODEX_NOTIFY_SUBCOMMAND => {
            ...
            if !auracoder_notification_env_available() {
                return Ok(true);
            }
...
fn parse_notify_cli_args(args: Vec<String>) -> anyhow::Result<Option<NotifyCliArgs>> {
    ...
    while index < args.len() {
        ...
        match flag {
            ...
            other => anyhow::bail!("unknown auracoder notify argument: {other}"),
        }
...
fn parse_clear_notification_cli_args(
    ...
    while index < args.len() {
        ...
        match flag {
            ...
            "--workspace-id" => parsed.workspace_id = value,
            "--session-id" => parsed.session_id = value,
            other => anyhow::bail!("unknown auracoder clear-notification argument: {other}"),
        }
...
fn parse_codex_notify_args(args: Vec<String>) -> anyhow::Result<Option<String>> {
    match args.as_slice() {
        ...
        _ => anyhow::bail!("auracoder codex-notify expects a single JSON payload argument"),
    }
}
...
fn handle_claude_hook(args: Vec<String>) -> anyhow::Result<()> {
    ...
    if !args.is_empty() {
        anyhow::bail!("auracoder claude-hook does not accept arguments");
    }
    if !auracoder_notification_env_available() {
        return Ok(());
    }
...
fn build_codex_forwarded_args(args: Vec<String>) -> anyhow::Result<Vec<String>> {
    if current_auracoder_session_id_if_available().is_none() {
        return Ok(args);
    }
...
fn build_claude_forwarded_args(args: Vec<String>) -> anyhow::Result<Vec<String>> {
    let Some(session_id) = current_auracoder_session_id_if_available() else {
        return Ok(args);
    };
...
fn current_auracoder_session_id_if_available() -> Option<String> {
    auracoder_notification_env_available()
...
fn is_managed_codex_notify_value(value: &toml::Value) -> bool {
    ...
    subcommand == CODEX_NOTIFY_SUBCOMMAND
        && (command == "auracoder" || command == managed_auracoder_cli_path_string())
}
...
fn managed_auracoder_cli_path() -> anyhow::Result<PathBuf> {
    Ok(install_cli_shims()?.join(auracoder_cli_shim_name()))
}

fn managed_auracoder_cli_path_string() -> String {
    runtime_env::app_data_dir()
        .join("bin")
        .join(auracoder_cli_shim_name())
        .to_string_lossy()
        .to_string()
}

fn claude_hook_command_for_config() -> anyhow::Result<String> {
    let auracoder_cli_path = managed_auracoder_cli_path()?;
    let path = auracoder_cli_path.to_string_lossy();
    #[cfg(windows)]
    {
...
fn codex_notify_config_value() -> anyhow::Result<toml::Value> {
    ...
    Ok(toml::Value::Array(vec![
        toml::Value::String(auracoder_cli_path.to_string_lossy().to_string()),
...
fn send_notify_request(
    ...
    let payload =
        serde_json::to_string(request).context("failed to serialize auracoder notify request")?;
    stream
        .write_all(payload.as_bytes())
        .context("failed to write auracoder notify request")?;
    stream
        .write_all(b"\n")
        .context("failed to finish auracoder notify request")?;

    let mut reader = BufReader::new(stream);
    let mut line = String::new();
    reader
        .read_line(&mut line)
        .context("failed to read auracoder notify response")?;
    if line.trim().is_empty() {
        return Ok(());
    }

    let response: NotificationIngressResponse =
        serde_json::from_str(line.trim()).context("failed to parse auracoder notify response")?;
    if response.ok {
        return Ok(());
    }
    ...
    anyhow::bail!(
        ...
        response
            .error
            .unwrap_or_else(|| "AuraCoder notification ingress rejected the request".to_string())
    );
}
...
fn install_cli_shims() -> anyhow::Result<PathBuf> {
    ...
    std::fs::create_dir_all(&bin_dir).with_context(|| {
        format!(
            "failed to create auracoder shim directory at {}",
            bin_dir.display()
        )
    })?;

    let current_exe =
        std::env::current_exe().context("failed to resolve current AuraCoder executable")?;
    write_cli_shim(
        &bin_dir.join(auracoder_cli_shim_name()),
        &auracoder_cli_shim_contents(&current_exe),
        "auracoder",
    )?;
    write_cli_shim(
...
#[cfg(windows)]
fn auracoder_cli_shim_name() -> &'static str {
    "auracoder.cmd"
}
...
#[cfg(windows)]
fn auracoder_cli_shim_contents(current_exe: &Path) -> String {
...
fn auracoder_notification_env_available() -> bool {
...
fn resolve_wrapped_binary(binary: &str, shim_dir: &Path) -> anyhow::Result<PathBuf> {
    ...
    which::which_in(binary, Some(filtered), cwd)
        .with_context(|| format!("failed to find the real {binary} binary outside AuraCoder shims"))
}
...
fn print_notify_help() {
    println!(
        "Usage: auracoder notify [--title TITLE] [--body BODY] [--workspace-id ID] [--session-id ID] [--source SOURCE]"
    );
}

fn print_clear_notification_help() {
    println!("Usage: auracoder clear-notification [--workspace-id ID] [--session-id ID]");
}

fn print_codex_notify_help() {
    println!("Usage: auracoder codex-notify '<codex notify JSON payload>'");
}

fn print_claude_hook_help() {
    println!("Usage: auracoder claude-hook");
}
...
mod tests {
    ...
    fn unix_cli_shim_escapes_single_quotes() {
        let contents = auracoder_cli_shim_contents(Path::new("/tmp/AuraCoder' Dev"));
        assert!(contents.contains("'/tmp/AuraCoder'\\'' Dev'"));
    }

    #[test]
    #[cfg(not(windows))]
    fn unix_claude_cli_shim_invokes_wrapper_subcommand() {
        let contents = claude_cli_shim_contents(Path::new("/tmp/AuraCoder' Dev"));
