...
#[cfg(target_os = "windows")]
fn preview_notification_sound_windows(app: tauri::AppHandle, sound: &str) -> Result<(), String> {
    let mut notification = app
        .notification()
        .builder()
        .title("AuraCoder")
...
pub async fn set_notification_sound(
    ...
    tokio::task::spawn_blocking(move || -> Result<String, String> {
        let normalized_sound = sound.trim().to_string();
        AppConfig::validate_notification_sound(&normalized_sound)?;
        let persisted_sound = if normalized_sound.is_empty() || normalized_sound == "none" {
            "none".to_string()
        } else {
            normalized_sound
        };
        ...
        AppConfig::mutate(|config| {
            config.general.notification_sound = Some(persisted_sound.clone());
            Ok(persisted_sound.clone())
        })
        .map_err(err_to_string)
    })
    .await
...
pub async fn preview_notification_sound(
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        return tokio::task::spawn_blocking(move || {
            preview_notification_sound_windows(app, &normalized_sound)
        })
        .await
        .map_err(err_to_string)?;
    }
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        AppConfig::validate_notification_sound(&normalized_sound)?;
        return tokio::task::spawn_blocking(move || {
            preview_notification_sound_macos(&normalized_sound)
        })
        .await
        .map_err(err_to_string)?;
    }
    ...
    #[cfg(target_os = "linux")]
    {
        let _ = (app, normalized_sound);
        return Err("当前系统暂不支持声音预览".to_string());
    }

    #[cfg(not(any(target_os = "windows", target_os = "macos", target_os = "linux")))]
    {
        let _ = (app, normalized_sound);
        Err("当前系统暂不支持声音预览".to_string())
    }
}
