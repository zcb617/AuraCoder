...
function surfaceLabel(t: TFunction<"app">, kind: WorkspacePaneSurfaceKind): string {
  return t(`workspaceAuraCoder.surfaces.${kind}`);
}
  ...
  if (!layout) {
    return (
      <div className="workspace-pane-shell workspace-pane-shell-empty">
        <span>{t("workspaceAuraCoder.loading")}</span>
      </div>
    );
  }
      ...
      >
        ...
        <div className="workspace-pane-header-actions no-drag">
          <div
            ...
            role="tablist"
            aria-label={t("workspaceAuraCoder.primarySwitcher")}
          >
            {SURFACE_ORDER.map((kind) => {
              const active = activeSurface === kind;
              const buttonLabel = t("workspaceAuraCoder.primarySurfaceTitle", {
      ...
      <div className="workspace-pane-canvas">
        {dockedGitPanel ? (
          <PanelGroup
            ...
            autoSaveId="auracoder:main-layout-panels"
  ...
  return (
    ...
    >
      <div style={{ maxWidth: 460 }}>
        ...
        <h2 style={{ margin: "0 0 9px", fontSize: 18 }}>{t("workspaceAuraCoder.remoteNoticeTitle")}</h2>
        <p style={{ margin: 0, color: "var(--text-3)", fontSize: 13, lineHeight: 1.65 }}>
          {connectionName ? `${connectionName} · ` : ""}
          {connectionEnabled === false
            ? t("workspaceAuraCoder.remoteNoticeDisabled")
            : t("workspaceAuraCoder.remoteNoticeDescription")}
        </p>
      </div>
    </div>
  );
...
export function ActiveWorkspacePaneShell(
  ...
  if (!activeWorkspaceId) {
    return (
      <div className="workspace-pane-shell workspace-pane-shell-empty">
        <span>{t("workspaceAuraCoder.noWorkspace")}</span>
      </div>
    );
  }
...
function PaneLeafView({
  ...
  const splitLabel =
    activeSurfaceKind === "chat"
      ? t("workspaceAuraCoder.splitWithEditor")
      : activeSurfaceKind === "editor"
        ? t("workspaceAuraCoder.splitWithChat")
        : null;
    ...
    >
      ...
      {leafCount > 1 && (
        <button
          ...
          title={t("workspaceAuraCoder.closePane")}
          aria-label={t("workspaceAuraCoder.closePane")}
        ...
        <div className="workspace-pane-snap-overlay" aria-hidden="true">
          ...
          >
            <div className="workspace-pane-snap-label">
              ...
              <span>
                {t(`workspaceAuraCoder.dropZones.${surfaceDrag.placement}`, {
...
function EmptyPane() {
  ...
  return (
    <div className="workspace-pane-empty">
      {t("workspaceAuraCoder.emptyPane")}
    </div>
  );
}
