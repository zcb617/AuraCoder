...
describe("workspaceStore.loadWorkspaces", () => {
  beforeEach(() => {
    ...
    const storage = {
      getItem: vi.fn((key: string) => {
        if (key === "auracoder:lastActiveWorkspaceId") {
          return "ws-mount";
        }
        return null;
      }),
  ...
  it("falls back from a transient AppImage workspace to the next valid workspace", async () => {
    const transientWorkspace = makeWorkspace("ws-mount", "/tmp/.mount_AuraCoder123/usr");
    const validWorkspace = makeWorkspace("ws-home", "/home/auracoder");
    const repo = makeRepo("repo-home", validWorkspace.id, "/home/auracoder/repo");
