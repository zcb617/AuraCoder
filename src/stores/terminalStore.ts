...
const LAYOUT_MODE_STORAGE_KEY = (wsId: string) => `auracoder:layoutMode:${wsId}`;
...
function isAbsolutePath(path: string): boolean {
  return path.startsWith("/") || /^[A-Za-z]:[\\/]/.test(path) || path.startsWith("ssh://auracoder/");
}
    ...
    for (const group of terminalPreset.groups) {
      ...
      for (const session of group.sessions) {
        try {
          ...
          warnings.push(`"${group.name}" was skipped because one of its auracoder could not be created.`);
          groupFailed = true;
          break;
        }
      }
