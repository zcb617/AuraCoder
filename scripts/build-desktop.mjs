...
const buildType = process.env.PANES_BUILD_TYPE ?? "production";

if (buildType !== "development" && buildType !== "production") {
  throw new Error(`PANES_BUILD_TYPE must be development or production, got: ${buildType}`);
}
...
await run(pnpmExecutable, ["run", "build"], {
  VITE_PANES_BUILD_TYPE: buildType,
});
